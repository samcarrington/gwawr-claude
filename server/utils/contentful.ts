...
export function getContentfulClient(
  ...
  const existingClient = isServer
    ? serverContentfulClient
    : clientContentfulClient;
  ...
  try {
    ...
    console.log(
      `[Contentful Service] Client created successfully (${isServer ? 'server' : 'client'})`
    );
    return client;
  } catch (error) {
    console.error(
      `[Contentful Service] Failed to create client (${isServer ? 'server' : 'client'}):`,
      error
    );
    return null;
  }
...
export function isContentfulConfigured(event?: any): boolean {
  try {
    ...
    return !!(
      config.public.contentfulSpaceId && config.public.contentfulAccessToken
    );
  } catch (error) {
...
export function getContentfulConfig(event?: any) {
  try {
    ...
    return {
      environment:
        typeof (globalThis as any).window === 'undefined' ? 'server' : 'client',
      spaceId: null,
