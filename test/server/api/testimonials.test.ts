...
describe('Testimonials API', () => {
  ...
  describe('/api/testimonials', () => {
    it('should fetch and transform testimonials successfully', async () => {
      const { transformTestimonials } = await import('~/utils/contentful-transformers')
      ...
      vi.mocked(transformTestimonials).mockResolvedValue(mockTransformedTestimonials)
      ...
      expect(transformTestimonials).toHaveBeenCalledWith(mockContentfulResponse.items)
    ...
    it('should throw error when Contentful is not configured', async () => {
      ...
      mockCreateError.mockReturnValue(new Error('Contentful configuration is missing'))
    ...
    it('should handle Contentful API errors', async () => {
      ...
      mockContentfulClient.getEntries.mockRejectedValue(new Error('Contentful API Error'))
      mockCreateError.mockReturnValue(new Error('Failed to fetch testimonials'))
  ...
  describe('/api/testimonials/featured', () => {
    it('should fetch and transform featured testimonials successfully', async () => {
      const { transformTestimonial } = await import('~/utils/contentful-transformers')
      ...
      mockContentfulClient.getEntries.mockResolvedValue(mockContentfulResponse)
      vi.mocked(transformTestimonial).mockResolvedValue(mockTransformedTestimonial)
      ...
      expect(transformTestimonial).toHaveBeenCalledWith(mockContentfulResponse.items[0])
      expect(mockSetHeader).toHaveBeenCalledWith(mockEvent, 'Cache-Control', 'public, max-age=900')
    ...
    it('should handle Contentful configuration errors', async () => {
      ...
      await expect(featuredTestimonialsHandler(mockEvent as any)).rejects.toThrow()
