...
export const useContentful = () => {
  ...
  const getBlogPosts = async (options?: {
    ...
    if (!client) {
      console.warn(
        '[useContentful] Client not available, returning empty array'
      );
      return [];
    }
    ...
    try {
      ...
      const activeClient =
        options?.preview && $contentfulPreview ? $contentfulPreview : client;
  ...
  const getBlogPostBySlug = async (
    ...
    try {
      ...
      const { $contentfulPreview } = useNuxtApp();
      const activeClient =
        preview && $contentfulPreview ? $contentfulPreview : client;
  ...
  const getFeaturedBlogPost = async (
    ...
    try {
      ...
      console.error(
        '[useContentful] Error fetching featured blog post:',
        error
      );
      return null;
    }
