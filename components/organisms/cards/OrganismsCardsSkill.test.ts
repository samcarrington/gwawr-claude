...
describe('OrganismsCardsSkill', () => {
  ...
  it('renders the icon with correct classes', async () => {
    ...
    const icon = wrapper.find('[data-testid="skill-icon"]') || wrapper.find('.w-8.h-8')
  ...
  it('validates required props', async () => {
    // Test that component requires all necessary props
    const requiredProps = ['icon', 'title', 'description', 'technologies']
    
    for (const propName of requiredProps) {
      const incompleteProps = { ...defaultProps }
      delete incompleteProps[propName as keyof typeof defaultProps]
      
      // This should throw or warn about missing required prop
      try {
        await mountSuspended(OrganismsCardsSkill, {
          props: incompleteProps
        })
      } catch (error) {
  ...
  it('renders with long content gracefully', async () => {
    const propsWithLongContent = {
      ...
      description: 'This is a very long description that contains multiple sentences and should test how the component handles longer text content. It should wrap appropriately and maintain good visual hierarchy.',
      technologies: ['Vue.js', 'TypeScript', 'Tailwind CSS', 'Nuxt.js', 'Vitest', 'ESLint', 'Prettier', 'Git']
    }
