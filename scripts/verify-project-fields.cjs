...
async function verifyProjectFields() {
  ...
  try {
    ...
    entries.items.forEach((entry, index) => {
      ...
      console.log(
        `   ✅ description: ${entry.fields.description ? 'Present' : 'Missing'}`
      );
      ...
      console.log('\n   New Fields:');
      console.log(
        `   ${entry.fields.repositoryUrl ? '✅' : '❌'} repositoryUrl: "${entry.fields.repositoryUrl || 'Not set'}"`
      );
      console.log(
        `   ${entry.fields.liveUrl ? '✅' : '⏭️'} liveUrl: "${entry.fields.liveUrl || 'Not set'}"`
      );
      console.log(
        `   ${entry.fields.featured !== undefined ? '✅' : '❌'} featured: ${entry.fields.featured}`
      );
      console.log(
        `   ${entry.fields.status ? '✅' : '❌'} status: "${entry.fields.status || 'Not set'}"`
      );
      console.log(
        `   ${entry.fields.endDate ? '✅' : '⏭️'} endDate: "${entry.fields.endDate || 'Not set'}"`
      );
      ...
      if (entry.fields.repositoryUrl) {
        ...
        console.log(
          `   ${urlPattern.test(entry.fields.repositoryUrl) ? '✅' : '❌'} Repository URL format: ${urlPattern.test(entry.fields.repositoryUrl) ? 'Valid' : 'Invalid'}`
        );
      }
      ...
      if (entry.fields.liveUrl) {
        const urlPattern = /^https?:\/\/.*/;
        console.log(
          `   ${urlPattern.test(entry.fields.liveUrl) ? '✅' : '❌'} Live URL format: ${urlPattern.test(entry.fields.liveUrl) ? 'Valid' : 'Invalid'}`
        );
      }
      ...
      if (entry.fields.status) {
        ...
        console.log(
          `   ${validStatuses.includes(entry.fields.status) ? '✅' : '❌'} Status validation: ${validStatuses.includes(entry.fields.status) ? 'Valid' : 'Invalid'}`
        );
      }
    ...
    console.log(
      `   Projects with repository URLs: ${entries.items.filter(e => e.fields.repositoryUrl).length}`
    );
    console.log(
      `   Projects with live URLs: ${entries.items.filter(e => e.fields.liveUrl).length}`
    );
    console.log(
      `   Projects with end dates: ${entries.items.filter(e => e.fields.endDate).length}`
    );
