...
async function findMatchingProject(
  ...
  try {
    ...
    for (const project of projects.items) {
      const projectTitle = (
        project.fields.title?.['en-US'] || ''
      ).toLowerCase();
      const projectDesc = (
        project.fields.description?.['en-US'] || ''
      ).toLowerCase();
...
async function populateTestimonialData() {
  ...
  try {
    ...
    console.log(
      `📋 Found ${entries.items.length} testimonial entries to update`
    );
    ...
    for (const [index, entry] of entries.items.entries()) {
      ...
      const projectReferenceId = await findMatchingProject(
        environment,
        clientName,
        clientCompany,
        quoteText
      ...
      console.log(
        `  🔗 Project Reference: ${projectReferenceId || 'None found'}`
      );
    ...
    console.log(
      `   - Featured testimonials: ${entries.items.filter((_, i) => i === 0).length}`
    );
    console.log(
      `   - Fields populated: clientName, clientTitle, clientCompany, rating, featured, projectReference`
    );
  } catch (error) {
