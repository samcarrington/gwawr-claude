...
async function verifyTestimonialFields() {
  ...
  try {
    ...
    entries.items.forEach((entry, index) => {
      ...
      console.log(`   ✅ quote: ${entry.fields.quote ? 'Present (RichText)' : 'Not set'}`)
      console.log(`   ✅ attribution: ${entry.fields.attribution ? 'Present (Person link)' : 'Not set'}`)
      
      // Check new fields
      console.log('\n   New Fields:')
      console.log(`   ${entry.fields.clientName ? '✅' : '❌'} clientName: "${entry.fields.clientName || 'Not set'}"`)
      console.log(`   ${entry.fields.clientTitle ? '✅' : '⏭️'} clientTitle: "${entry.fields.clientTitle || 'Not set'}"`)
      console.log(`   ${entry.fields.clientCompany ? '✅' : '⏭️'} clientCompany: "${entry.fields.clientCompany || 'Not set'}"`)
      console.log(`   ${entry.fields.rating ? '✅' : '❌'} rating: ${entry.fields.rating || 'Not set'}`)
      console.log(`   ${entry.fields.featured !== undefined ? '✅' : '❌'} featured: ${entry.fields.featured}`)
      console.log(`   ${entry.fields.projectReference ? '✅' : '⏭️'} projectReference: ${entry.fields.projectReference ? 'Present' : 'Not set'}`)
      ...
      if (entry.fields.rating) {
        const validRating = entry.fields.rating >= 1 && entry.fields.rating <= 5
        console.log(`   ${validRating ? '✅' : '❌'} Rating range: ${validRating ? 'Valid (1-5)' : 'Invalid'} (${entry.fields.rating})`)
      }
    ...
    console.log(`   Testimonials with client names: ${entries.items.filter(e => e.fields.clientName).length}`)
    console.log(`   Testimonials with ratings: ${entries.items.filter(e => e.fields.rating).length}`)
    console.log(`   Testimonials with project references: ${entries.items.filter(e => e.fields.projectReference).length}`)
