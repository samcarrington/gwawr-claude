...
class DataPopulationOrchestrator {
    ...
    try {
      ...
      for (const [index, entry] of entries.items.entries()) {
        try {
          ...
          // Determine category
          const category = this.determineBlogCategory(title, entry.fields.content?.['en-US'])
          
          // Generate tags
          const tags = this.generateBlogTags(title, entry.fields.content?.['en-US'], category)
          
          // Calculate read time
          const readTime = this.calculateReadingTime(entry.fields.content?.['en-US'])
      ...
      console.log(`✅ BlogPost population completed: ${this.results.blogPost.processed} processed, ${this.results.blogPost.errors} errors`)
    ...
    try {
      ...
      for (const [index, entry] of entries.items.entries()) {
        try {
          ...
          let liveUrl = null
          if (title.toLowerCase().includes('website') || title.toLowerCase().includes('app') || title.toLowerCase().includes('guide')) {
            liveUrl = `https://${slug}.samcarrington.dev`
          }
          
          // Determine featured status
          const featured = featuredCount < maxFeatured && (index < 2 || title.toLowerCase().includes('premier'))
      ...
      console.log(`✅ Project population completed: ${this.results.project.processed} processed, ${this.results.project.errors} errors`)
  ...
  async populateTestimonials() {
    ...
    try {
      ...
      console.log(`✅ Testimonial population completed: ${this.results.testimonial.processed} processed, ${this.results.testimonial.errors} errors`)
  ...
  determineBlogCategory(title, content) {
    const text = (title + ' ' + this.extractTextFromRichText(content)).toLowerCase()
  ...
  generateBlogTags(title, content, category) {
    ...
    const potentialTags = ['javascript', 'vue', 'css', 'api', 'architecture', 'development']
  ...
  printSummary() {
    ...
    for (const [contentType, result] of Object.entries(this.results)) {
      console.log(`   ${contentType}: ${result.processed} processed, ${result.errors} errors`)
