...
class FieldRequirementsUpdater {
  ...
  async validateDataIntegrity(contentTypeName, fieldId) {
    console.log(`   🔍 Validating data integrity for ${contentTypeName}.${fieldId}...`)
    ...
    try {
      ...
      const entriesWithoutField = entries.items.filter(entry => {
        ...
        return !fieldValue || !fieldValue['en-US'] || 
               (typeof fieldValue['en-US'] === 'string' && fieldValue['en-US'].trim() === '')
      })
      
      if (entriesWithoutField.length > 0) {
        console.log(`   ⚠️  Found ${entriesWithoutField.length} entries without ${fieldId}:`)
      ...
      console.log(`   ✅ All ${entries.items.length} entries have valid ${fieldId} values`)
      return true
  ...
  async updateFieldRequirement(contentTypeName, fieldId, reason) {
    ...
    try {
      ...
      const isDataValid = await this.validateDataIntegrity(contentTypeName, fieldId)
      if (!isDataValid) {
      ...
      const contentType = await this.environment.getContentType(contentTypeName)
      
      // Find the field
      const fieldIndex = contentType.fields.findIndex(field => field.id === fieldId)
      if (fieldIndex === -1) {
      ...
      // Save and publish
      console.log(`   💾 Saving ${contentTypeName} with updated field requirement...`)
      const updatedContentType = await contentType.update()
      
      console.log(`   📤 Publishing ${contentTypeName}...`)
      await updatedContentType.publish()
      
      console.log(`   ✅ Successfully made ${fieldId} required in ${contentTypeName}`)
  ...
  printSummary() {
    ...
    if (totalFailed > 0) {
      ...
      console.log('   This usually means entries exist without values for these fields.')
      console.log('   Run Phase 2 data population again or manually fix the entries.')
    }
  ...
  async run() {
    ...
    try {
      ...
      for (const [contentTypeName, fieldsToUpdate] of Object.entries(REQUIRED_FIELD_UPDATES)) {
