...
function determineCategory(title, content) {
  ...
  const contentText = extractTextFromRichText(
    content?.content || []
  ).toLowerCase();
...
async function populateBlogPostData() {
  ...
  try {
    ...
    console.log(
      `   - Fields populated: slug, category, tags, readTime, publishedAt, featured`
    );
    console.log(
      `   - Featured posts: ${entries.items.filter((_, i) => i === 0).length}`
    );
  } catch (error) {
