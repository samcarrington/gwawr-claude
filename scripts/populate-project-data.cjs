...
function generateLiveUrl(slug, title) {
  ...
  if (titleLower.includes('website') || titleLower.includes('app') || titleLower.includes('guide')) {
...
function shouldBeFeatured(title, description, index, totalProjects) {
  ...
  return projectUpdates.featuredKeywords.some(keyword => 
...
async function populateProjectData() {
  ...
  try {
    ...
    for (const [index, entry] of entries.items.entries()) {
      ...
      const description = entry.fields.description ? entry.fields.description['en-US'] : ''
      const startDate = entry.fields.date ? entry.fields.date['en-US'] : entry.sys.createdAt
      
      // Generate new field values
      const repositoryUrl = generateRepositoryUrl(slug, title)
      const liveUrl = generateLiveUrl(slug, title)
      const featured = featuredCount < maxFeatured && shouldBeFeatured(title, description, index, entries.items.length)
    ...
    console.log(`   - Fields populated: repositoryUrl, liveUrl, featured, status, endDate`)
