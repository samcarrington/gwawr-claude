...
async function addTestimonialFields() {
  ...
  try {
    ...
    console.log(
      `✅ Found testimonial content type with ${contentType.fields.length} existing fields`
    );
    ...
    if (fieldsAdded > 0) {
      ...
      console.log(
        `✅ Successfully added ${fieldsAdded} fields to testimonial content type!`
      );
    } else {
    ...
    finalContentType.fields.forEach((field, index) => {
      ...
      console.log(
        `   ${index + 1}. ${field.name} (${field.id}) - ${field.type} ${isNew ? '🆕' : ''}`
      );
    });
  } catch (error) {
