...
class MigrationOrchestrator {
  ...
  async createBackup() {
    ...
    try {
      ...
      const backupFile = path.join(__dirname, `backup-content-types-${Date.now()}.json`)
    ...
    const checks = [
      {
        ...
        message: 'CONTENTFUL_SPACE_ID and CONTENTFUL_MANAGEMENT_TOKEN must be set'
      },
      {
      ...
      },
      {
        ...
        message: 'Required content types (blogPost, project, testimonial) not found'
      }
    ]
  ...
  async executePhase(phaseId, options = {}) {
    ...
    try {
      if (phase.manual) {
        console.log('📖 This is a manual phase. Please refer to the documentation:')
        ...
        if (!options.skipConfirmation) {
          ...
          await new Promise((resolve) => {
            rl.question('Press Enter when manual tasks are completed...', () => {
              rl.close()
              resolve()
            })
          })
  ...
  async executeFullMigration(options = {}) {
    ...
    try {
      ...
      for (const phaseId of phasesToExecute) {
        ...
        if (options.pauseBetweenPhases && phaseId !== phasesToExecute[phasesToExecute.length - 1]) {
...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log('  --pause-between-phases Pause between phases for verification')
    console.log('')
    console.log('Examples:')
    console.log('  node migration-orchestrator.cjs full')
    console.log('  node migration-orchestrator.cjs phase phase1')
    console.log('  node migration-orchestrator.cjs full --skip-backup --pause-between-phases')
    return
  }
