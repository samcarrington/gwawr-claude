...
async function testContentfulIntegration() {
  ...
  try {
    ...
    entries.items.forEach(entry => {
      console.log(`  - ${entry.fields.title || entry.fields.name || 'Untitled'} (${entry.sys.contentType.sys.id})`)
    })
    console.log()
    ...
    try {
      ...
      console.log(`✅ Found ${blogPosts.total} blog posts (showing first ${blogPosts.items.length}):`)
      blogPosts.items.forEach(post => {
        console.log(`  - ${post.fields.title} (${post.fields.slug})`)
      })
    } catch (error) {
      console.log('ℹ️  Blog post content type may not exist yet:', error.message)
    }
    console.log()
