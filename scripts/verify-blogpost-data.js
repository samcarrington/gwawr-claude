...
async function verifyBlogPostData() {
  ...
  try {
    ...
    for (const entry of entries.items) {
      ...
      console.log(`\n   🏷️ Field Values:`);
      console.log(
        `   - slug: ${entry.fields.slug ? entry.fields.slug['en-US'] : 'NOT SET'}`
      );
      console.log(
        `   - category: ${entry.fields.category ? entry.fields.category['en-US'] : 'NOT SET'}`
      );
      console.log(
        `   - tags: ${entry.fields.tags ? entry.fields.tags['en-US'].join(', ') : 'NOT SET'}`
      );
      console.log(
        `   - readTime: ${entry.fields.readTime ? entry.fields.readTime['en-US'] : 'NOT SET'}`
      );
      console.log(
        `   - publishedAt: ${entry.fields.publishedAt ? entry.fields.publishedAt['en-US'] : 'NOT SET'}`
      );
      console.log(
        `   - featured: ${entry.fields.featured ? entry.fields.featured['en-US'] : 'NOT SET'}`
      );
      console.log(`\n   📊 Field Validation:`);
      ...
      if (entry.fields.slug) {
        ...
        console.log(
          `   - slug pattern (a-z0-9-): ${slugPattern.test(slug) ? '✅ VALID' : '❌ INVALID'}`
        );
      }
      ...
      if (entry.fields.category) {
        ...
        console.log(
          `   - category options: ${validCategories.includes(category) ? '✅ VALID' : '❌ INVALID'}`
        );
      }
      ...
      if (entry.fields.tags) {
        ...
        console.log(
          `   - tags array: ${Array.isArray(tags) ? '✅ VALID' : '❌ INVALID'}`
        );
      ...
      if (entry.fields.readTime) {
        ...
        console.log(
          `   - readTime number: ${typeof readTime === 'number' && readTime > 0 ? '✅ VALID' : '❌ INVALID'}`
        );
      }
      ...
      if (entry.fields.publishedAt) {
        ...
        console.log(
          `   - publishedAt date: ${publishedAt && new Date(publishedAt).toString() !== 'Invalid Date' ? '✅ VALID' : '❌ INVALID'}`
        );
      }
      ...
      if (entry.fields.featured !== undefined) {
        ...
        console.log(
          `   - featured boolean: ${typeof featured === 'boolean' ? '✅ VALID' : '❌ INVALID'}`
        );
      }
