...
async function analyzeSampleEntries() {
  ...
  try {
    ...
    for (const contentType of contentTypes.items) {
      console.log(`📝 Sample entries for: ${contentType.name} (${contentType.sys.id})`)
      ...
      try {
        ...
        if (entries.items.length === 0) {
          ...
          console.log(`   📊 Found ${entries.total} total entries (showing first ${entries.items.length}):`)
          ...
          entries.items.forEach((entry, index) => {
            ...
            for (const [fieldId, fieldValue] of Object.entries(entry.fields)) {
              ...
              if (fieldValue === null || fieldValue === undefined) {
                ...
                const preview = fieldValue.length > 50 ? fieldValue.substring(0, 50) + '...' : fieldValue
                ...
                if (fieldValue.sys.type === 'Link') {
                  console.log(`       ${fieldName} (${fieldId}): Link to ${fieldValue.sys.linkType} (${fieldValue.sys.id})`)
                } else if (fieldValue.sys.type === 'Entry') {
                  console.log(`       ${fieldName} (${fieldId}): Entry ${fieldValue.sys.contentType.sys.id} - "${fieldValue.fields.title || fieldValue.fields.name || 'No title'}"`)
                } else if (fieldValue.sys.type === 'Asset') {
                  console.log(`       ${fieldName} (${fieldId}): Asset "${fieldValue.fields.title}" (${fieldValue.fields.file.url})`)
                }
              } else if (Array.isArray(fieldValue)) {
                console.log(`       ${fieldName} (${fieldId}): Array with ${fieldValue.length} items:`)
                fieldValue.forEach((item, i) => {
                  if (typeof item === 'string') {
                    console.log(`         [${i}]: "${item}"`)
                  } else if (item && item.sys) {
                    if (item.sys.type === 'Entry') {
                      console.log(`         [${i}]: Entry ${item.sys.contentType.sys.id} - "${item.fields.title || item.fields.name || 'No title'}"`)
                    } else if (item.sys.type === 'Asset') {
                      console.log(`         [${i}]: Asset "${item.fields.title || 'No title'}"`)
                    }
                  } else {
                    console.log(`         [${i}]: ${JSON.stringify(item)}`)
                  }
                })
              } else if (typeof fieldValue === 'object' && fieldValue.nodeType) {
                // This is a rich text document
                console.log(`       ${fieldName} (${fieldId}): Rich Text Document`)
                console.log(`         Node Type: ${fieldValue.nodeType}`)
                if (fieldValue.content && fieldValue.content.length > 0) {
                  console.log(`         Content: ${fieldValue.content.length} nodes`)
                ...
                console.log(`       ${fieldName} (${fieldId}): ${JSON.stringify(fieldValue)}`)
              }
            }
          })
        }
